...
function getHandType(hand: string) {
  if (hand.includes("J")) {
    ...
    let bestHand: string | undefined;
    for (const c of otherCards) {
      const newHand = getHandType(hand.replace(/J/g, c));
      if (!bestHand) {
        bestHand = newHand;
      } else if (powerMap[bestHand] < powerMap[newHand]) {
        bestHand = newHand;
      }
    }
    return bestHand as string;
  }
...
function run(input: string) {
  ...
  games.sort(compare);
  console.info(games.filter((g) => g.handType === "TwoPair"));
